...
class AnalysisWindow(Toplevel):
	...
	def initwindow(self):
		...
		OptionMenu(leftFrame, self.graphmode, "Average percentage displacement", "Percentage displacement", 
			"Average percentage displacement", "Voltages", "Swelling rate", "Average swelling rate",
			 "Total swell", "MMMGEW", command=self.setGraphMode).pack(side=LEFT, pady=(4, 0))
		self.setGraphMode()
			...
			try:
				if os.path.exists(foldername):
					if not os.path.isdir(foldername) or os.listdir(foldername): #is a file or is nonempty
						if not messagebox.askyesno("Delete", "'" + os.path.basename(foldername) + "' already exists. Delete?", parent=self):
							return
				...
				messagebox.showerror("Error", "Failed to generate report. Please make sure nothing is using '" + os.path.basename(foldername) + "'", parent=self)
				raise e
	...
	def deleteObject(self, iid, warn=True):
		...
		if "runname" in pointed:
			if warn:
				res = messagebox.askyesno("Remove run", "Are you sure you want to remove this run and all of its data?", parent=self)
				if res == "no":
					return
